...
let tweetsFromLocalStorage = JSON.parse(localStorage.getItem('data'))
...
function deleteTweet(tweetId){
    const targetTweet = tweets.filter(function(tweet) {
        return tweet.uuid === tweetId;
    })[0]

    tweets.splice(tweets.indexOf(targetTweet), 1)
    localStorage.setItem("data", JSON.stringify(tweets))
    render()
}
...
function handleReplyBtnClick(tweetId){
  ...
  tweets.forEach(function(tweet){
    if (tweet.uuid === tweetId){
      ...
      tweet.replies.push(newReplyObj)
      localStorage.setItem("data", JSON.stringify(tweets))
      replyMessage.value = ''
